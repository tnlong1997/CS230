...
int atm (int bank_out_fd, int atm_in_fd, int atm_id, Command *cmd) {
  ...
  if (atm_id != i) return ERR_UNKNOWN_ATM;
  ...
  if (status == SUCCESS) {
    ...
    if (status == SUCCESS) {
      ...
      if (c== OK) status = SUCCESS;
      else if (c == ACCUNKN) status = ERR_UNKNOWN_ACCOUNT;
      else if (c == NOFUNDS) status = ERR_NOFUNDS;
      else {
