...
WordSet *wset_new () {
  ...
  WordSet *c = (WordSet *)(malloc(sizeof(WordSet)));
  c -> head = NULL;
  c -> size = 0;
  return c;
}
...
void wset_free (WordSet *wset) {
  ...
  if (wset->head != NULL) node_free_all(wset->head);
}
...
static WordNode **search (WordSet *wset, char *word) {
  ...
  int flag = 0;
  while ((*p)!=NULL && (flag == 0)) {
    if (strcmp(word, (*p) -> word) == 0) flag =1;
    else p = &((*p) -> next);
  }
  return p;
}
...
int wset_add (WordSet *wset, char *word) {
  ...
  if ((*p) != NULL) {
    ...
    WordNode **t = &(wset -> head);
    while ((*t) != NULL) t = &((*t) -> next);
    WordNode *nn = node_new(word);
    *t = nn;
    nn -> freq = 1;
    wset->size += 1;
    return 1;
...
int wset_exists (WordSet *wset, char *word) {
  ...
  //WordNode **p = search(wset,word);
  if (*(search(wset,word)) == NULL) return 0; else return 1;
}
